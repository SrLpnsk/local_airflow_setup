...
case "$1" in
  webserver)
    ...
    airflow db upgrade
    airflow connections import config_files/connections.json
    airflow users create --role Admin --username admin --firstname ${FIRSTNAME} --lastname ${LASTNAME} --email ${EMAIL} --password admin
    exec airflow webserver
    ;;
  celery|scheduler|flower)
